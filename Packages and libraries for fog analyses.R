...
##Data cleaning
#***************************************************************************************************
setwd("C:/Users/ABRI/Documents/SAEON/Projects/Fog project_Abri/Data for analyses/Raw data")
dat<-read.csv("Langrivier Fog 800.csv",skip=0) ##edit your .csv file to only include one row of column headers before reading it in
...
ts <- seq.POSIXt(as.POSIXct("2014/05/19 15:00",'%m/%d/%y %H:%M'), as.POSIXct("2017/10/01 00:00",'%m/%d/%y %H:%M'), by="hour")  ##Insert the start and end TIMESTAMPS of your raw data. This creates a TIMESTAMP column without any missing hours
...
dat<-read.csv("F8duplicateless.csv",skip=0) ##Read in whatever raw data file you want to work on
...
write.csv(data_with_missing_times, "C:/Users/ABRI/Documents/SAEON/Projects/Fog project_Abri/Data for analyses/Raw data/Fog800_clean.csv", sep="\t", col.names=T,quote=F, row.names=T, na = "NA")
